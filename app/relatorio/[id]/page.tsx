...
type PackageRow = RowDataPacket & { id: number; name: string; totalSessions: number; totalAmountCents: number; paidAmountCents: number; paymentStatus: string; purchasedAt: string | Date; status: string };
...
export default async function PatientReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const packages = await rows<PackageRow[]>("SELECT id, name, total_sessions AS totalSessions, total_amount_cents AS totalAmountCents, paid_amount_cents AS paidAmountCents, payment_status AS paymentStatus, purchased_at AS purchasedAt, status FROM packages WHERE patient_id = ? AND owner_id = ? ORDER BY purchased_at DESC, id DESC", [id, ownerId]);
  ...
  return <main className="min-h-screen bg-[#f4f7f5] px-4 py-8 text-[#17231f] print:bg-white print:p-0">
    <div className="mx-auto max-w-4xl rounded-[28px] border bg-white p-6 shadow-sm sm:p-10 print:max-w-none print:border-0 print:p-0 print:shadow-none">
      ...
      <section className="mt-9"><Title>Pacotes</Title><div className="mt-3 overflow-hidden rounded-xl border"><table className="w-full text-left text-sm"><thead className="bg-[#f4f7f5] text-xs uppercase text-[#6f7d75]"><tr><th className="p-3">Pacote</th><th className="p-3">Contratação</th><th className="p-3">Sessões</th><th className="p-3">Pagamento</th></tr></thead><tbody className="divide-y">{packages.map((pkg) => <tr key={pkg.id}><td className="p-3"><strong>{pkg.name}</strong><span className="block text-xs text-[#748179]">{pkg.status}</span></td><td className="p-3">{dateBR(pkg.purchasedAt)}</td><td className="p-3">{sessions.filter((item) => item.packageId === pkg.id).length} de {pkg.totalSessions}</td><td className="p-3">{money(pkg.paidAmountCents)} de {money(pkg.totalAmountCents)}<span className="block text-xs text-[#748179]">{pkg.paymentStatus}</span></td></tr>)}{!packages.length && <tr><td colSpan={4} className="p-5 text-center text-[#758179]">Nenhum pacote cadastrado.</td></tr>}</tbody></table></div></section>
      <section className="mt-9"><Title>Histórico de sessões</Title><div className="mt-3 grid gap-3">{sessions.map((session, index) => <article key={session.id} className="break-inside-avoid rounded-xl border p-4"><div className="flex items-start justify-between gap-3"><div><strong>{sessions.length - index}ª sessão · {session.procedureType || "Atendimento"}</strong><p className="text-xs text-[#758179]">{packageName(session.packageId)}</p></div><strong className="text-sm text-[#087052]">{dateBR(session.performedAt)}</strong></div>{(session.measurementIn || session.measurementOut) && <p className="mt-3 text-sm"><strong>Medidas:</strong> entrada {session.measurementIn || "—"} · saída {session.measurementOut || "—"}</p>}{session.notes && <p className="mt-2 whitespace-pre-wrap text-sm"><strong>Evolução e observações:</strong> {session.notes}</p>}{session.occurrences && <p className="mt-2 whitespace-pre-wrap rounded-lg bg-amber-50 p-2 text-sm"><strong>Intercorrências:</strong> {session.occurrences}</p>}</article>)}{!sessions.length && <p className="rounded-xl border p-5 text-center text-[#758179]">Nenhuma sessão registrada.</p>}</div></section>
      <section className="mt-9"><Title>Pagamentos</Title><div className="mt-3 overflow-hidden rounded-xl border"><table className="w-full text-left text-sm"><thead className="bg-[#f4f7f5] text-xs uppercase text-[#6f7d75]"><tr><th className="p-3">Data</th><th className="p-3">Pacote</th><th className="p-3">Forma</th><th className="p-3 text-right">Valor</th></tr></thead><tbody className="divide-y">{payments.map((payment) => <tr key={payment.id}><td className="p-3">{dateBR(payment.paidAt)}</td><td className="p-3">{packageName(payment.packageId)}</td><td className="p-3">{payment.method}</td><td className="p-3 text-right font-semibold">{money(payment.amountCents)}</td></tr>)}{!payments.length && <tr><td colSpan={4} className="p-5 text-center text-[#758179]">Nenhum pagamento registrado.</td></tr>}</tbody></table></div></section>
