...
export default async function PatientReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const coverage = calculateSessionCoverage(
    packages.map((pkg) => ({ id: Number(pkg.id), totalSessions: Number(pkg.totalSessions), totalAmountCents: Number(pkg.totalAmountCents), paidAmountCents: Number(pkg.paidAmountCents) })),
