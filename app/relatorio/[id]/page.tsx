...
export default function PatientReportPage() {
  ...
  return <main className="min-h-screen bg-[#f4f7f5] px-4 py-8 text-[#17231f] print:bg-white print:p-0">
    <div className="mx-auto max-w-4xl rounded-[28px] border bg-white p-6 shadow-sm sm:p-10 print:max-w-none print:border-0 print:p-0 print:shadow-none">
      ...
      <header className="flex flex-wrap items-start justify-between gap-5 border-b-2 border-[#07533f] pb-6"><div className="flex items-center gap-4"><BrandLogo size={72} className="rounded-2xl border" /><div><p className="text-sm font-semibold uppercase tracking-[.16em] text-[#087052]">Dra. Maria Carolini · Fisioterapeuta</p><h1 className="mt-2 font-serif text-3xl font-semibold">Relatório do paciente</h1><p className="mt-1 text-sm text-[#6f7d75]">Histórico de sessões, pacotes e pagamentos</p></div></div><div className="text-right text-sm"><strong className="block">Dra. Maria Carolini</strong><span className="text-[#758179]">Emitido em {new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(new Date())}</span></div></header>
      ...
      <footer className="mt-10 border-t pt-5 text-xs text-[#77847c]">Documento profissional da Dra. Maria Carolini, fisioterapeuta. As informações refletem os registros cadastrados até a data da emissão.</footer>
    </div>
  </main>;
}
